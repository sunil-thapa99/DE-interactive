...
mlai: {
  ...
  cards: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"Beyond RAG, the current Mosaic AI layer is the Agent Framework — for building and evaluating tool-calling, multi-step agents with MLflow tracing and Agent Evaluation — and Agent Bricks, the more auto-optimized way to stand up domain agents over governed data. In front of the models sits AI Gateway, the governance proxy that gives rate limiting, usage tracking, and PII/safety guardrails with one managed credential. And Lakebase is managed Postgres inside the Lakehouse, so operational app state lives beside analytical Delta under one governance model — which matters because agents and real-time apps need an OLTP store, not just Delta. My role stays consistent: I don't build the agent's reasoning, I own its data plumbing — the governed tables and Vector Search indexes it retrieves from, the online features it calls, the Gateway config governing its model access, and the pipelines keeping Lakebase and Delta in sync. The agent is only as good as the governed, fresh data behind it, and that's mine.\"",
      followups: [
        { q: "\"What's the difference between Agent Framework and Agent Bricks?\"", a: "Agent Framework is the lower-level toolkit — you compose tools, retrieval, and chains and evaluate them with MLflow tracing/Agent Evaluation. Agent Bricks is the higher-level, auto-optimized path that builds a domain agent from your governed data with far less hand-tuning. Same platform, different level of control vs automation." },
        { q: "\"Why would you put Mosaic AI Gateway in front of a model endpoint?\"", a: "Governance and observability: centralized rate limiting, usage/cost tracking, PII and safety guardrails, and one credential-managed entry point for both foundation and external models — so model access is governed like any other resource instead of scattered API keys in notebooks." },
        { q: "\"What is Lakebase and why does it exist on a lakehouse platform?\"", a: "It's managed Postgres (OLTP) integrated into the Lakehouse, so transactional/operational state — app data, agent state, serving lookups — sits next to analytical Delta under one governance model, instead of bolting on a separate database. It closes the operational gap Delta alone doesn't cover." },
        { q: "\"As a DE, what do you own for an LLM agent versus what the ML team owns?\"", a: "I own the data plumbing: the governed tables and Vector Search indexes it retrieves from, fresh online features, the Gateway config governing its model access, and syncing operational (Lakebase) with analytical (Delta) state. The ML team owns the agent's reasoning, tools wiring, and prompts. The agent's reliability is downstream of my data freshness and governance." }
      ]
    },
    {
