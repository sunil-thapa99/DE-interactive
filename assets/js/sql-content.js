  ...
  cards: [
    {
      ...
      note: "\"Before I write anything: what's the grain of the input and the expected output, how should I treat NULLs and duplicate rows, and how do I break ties? Then I'll write it in CTEs so it reads top-to-bottom, and tell you where it'd get expensive at scale.\" That sentence alone signals seniority.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"A window function keeps the rows and computes something over a partition; GROUP BY collapses them. I default to windows when I need per-row context — rank, prior value, running total — and to GROUP BY when I only need the aggregate, because it's cheaper.\"",
      followups: [
  ...
  cards: [
    {
      ...
      noteLabel: "Model answer:",
      note: "The gotcha: ROW_NUMBER with a non-unique ORDER BY is non-deterministic — two runs can pick different 'winners' among ties. If the choice matters (e.g. which duplicate record survives dedup), add a deterministic tiebreaker like ORDER BY amount DESC, txn_id. Interviewers plant tied values specifically to see if you notice.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "Why not GROUP BY member_id, MAX(updated_at) then join back? Because if two rows share the same member_id AND updated_at you get duplicate winners — the join fans out. ROW_NUMBER with a tiebreaker (source_file_seq) guarantees exactly one row. In dbt this is the canonical 'latest snapshot' pattern; the tiebreaker is what makes it idempotent across reruns.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "Know both forms. In an interview I'll write the portable CTE version first ('this runs anywhere'), then mention 'in Snowflake I'd collapse it to QUALIFY.' Reaching straight for QUALIFY on a Postgres interview (it doesn't support it) is a small but real tell that you conflate dialects.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "The failure mode: forgetting the third arg (default) so the first row's delta is NULL and silently drops out of a later WHERE. In fraud/velocity work, gap_since_prev being unexpectedly NULL for a customer's first transaction is a real bug — I set a sensible default or handle the NULL explicitly.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "The subtle bug: relying on the default frame with ORDER BY gives you RANGE, so if two rows share the same txn_date they get the SAME running total (both include each other) — not what you want for a strict cumulative sum. Specify ROWS explicitly when the grain has ties. This exact issue produces 'my running total has duplicate values' bug reports.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "If the interviewer says 'include ties at the cutoff' switch ROW_NUMBER to RANK — now 'top 3' with a 3-way tie for 3rd returns 5 rows, which may be the desired business rule. Always clarify: 'top 3 rows, or top 3 distinct values?' The word 'top' is ambiguous and they're testing whether you catch it.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "The intuition to say out loud: 'within a run of consecutive dates, date minus its row number is constant, because both increase in lockstep; a gap breaks the lockstep and starts a new group.' This handles healthcare 'continuous enrollment' and 'find the longest active streak' identically. Watch for duplicate dates — dedup first or the row-number arithmetic drifts.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "The two edge cases interviewers check: the FIRST event per user (LAG is NULL — must count as a new session, hence the IS NULL branch) and the frame on the running sum (must be ROWS ... CURRENT ROW so the session id increments correctly). Miss the NULL branch and every user's first session is unlabeled.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "The two traps: (1) using b.txn_ts > a.txn_ts (strict, ordered) instead of <> avoids both self-pairs (a=a) and mirror duplicates (a,b and b,a). (2) A self-join is O(n^2) per partition — fine for pairs-within-5-minutes on a partitioned key, but a self-join without a selective ON condition on a big table is a query that never finishes. Name the cost.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "The NOT IN + NULL trap is the single most common silent-empty-result bug in production SQL. I default to NOT EXISTS or a LEFT JOIN ... IS NULL anti-join, which are NULL-safe and usually plan better. For equality that should treat NULL as a value, use IS NOT DISTINCT FROM (Postgres) / EQUAL_NULL (Snowflake).",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "Always ask 'do missing periods need to appear as zero?' For a running total or moving average the answer is almost always yes — otherwise a quiet day just vanishes and skews the trend. The calendar/date-dimension LEFT JOIN is the standard fix; in Snowflake I'd use a date dimension table, in Postgres generate_series.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "The honest senior point: SQL pivots are fixed-width — you must know the target columns at write time. If the set of pivot columns is dynamic (unknown merchant categories), don't fight SQL: pivot in the presentation layer, in Spark, or generate the SQL programmatically. Conditional aggregation is more portable and more flexible than the PIVOT keyword.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "Reach for EXISTS/NOT EXISTS over IN/NOT IN because they're NULL-safe (NOT IN breaks on NULLs) and they short-circuit. Reach for them over a JOIN when you only need existence, not B's columns — a JOIN duplicates the left row per match, which quietly inflates counts and sums. 'Why is my total double?' is often a semi-join written as a JOIN.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "The framing that lands: \"I don't guess — I read EXPLAIN and find the biggest node: a full scan I can prune, a shuffle I can broadcast away, or a spill I can fix with more partitions. The cheapest query reads the least data, so partition pruning and column projection come first; join strategy second.\" Naming 'a function on the partition column defeats pruning' is a concrete senior signal — it's the #1 reason a 'filtered' query still scans everything.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "This is my reflex when a join produces more rows than expected: check both sides for duplicate join keys with GROUP BY ... HAVING COUNT(*) > 1 before touching the join itself. The fan-out is almost always a non-unique key on one side, not a bug in the JOIN. Cheap to run, and it's the first thing I'd show an interviewer as my debugging instinct.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"Type 1 overwrites — I use it when history has no business value, like fixing a typo. Type 2 keeps history: surrogate key, effective_from/to, is_current, and facts join on the surrogate key so an old fact still resolves to the attributes that were true then. The two gotchas I call out unprompted: (1) diff on a hash of the tracked columns, not row-by-row, so a reload of unchanged data doesn't create phantom versions and break idempotency; (2) a single MERGE can't close the old row and insert its successor at once — you close in the MERGE and insert successors in a second step, or the new version silently overwrites the close. At Cedar Gate that mattered for provider network-status changes: a claim adjudicated under the old status must never re-resolve to the new one.\" — The remaining follow-ups, answered crisply: (a) Surrogate key, not natural key, because a Type 2 dimension has MANY rows per business key; joining facts on member_id would fan out to every version — the surrogate key uniquely identifies ONE version, so a fact resolves to exactly the attributes true at that time. It also decouples the warehouse from source-key changes/reuse. (b) When to pick each: Type 1 (overwrite) when history has no business value — typo fixes, backfilled nulls; Type 3 (add a 'previous_value' column) only when you need exactly one prior value and want it side-by-side, rare; a full daily snapshot table when nearly every attribute changes often or you need point-in-time on the WHOLE row cheaply — simpler than Type 2 but storage-heavy, so reserve it for small dimensions. Type 2 is the default when history matters and changes are sparse. (c) dbt snapshots implement exactly this: strategy='timestamp' (updated_at column) is the effective-dating driver, strategy='check' (check_cols) is the hash-diff I showed by hand; dbt manages dbt_valid_from / dbt_valid_to / dbt_scd_id (the surrogate) for you — same model, generated.",
      followups: [
  ...
  cards: [
    {
      ...
      noteLabel: "Model answer:",
      note: "The lazy-evaluation point is the one to lead with: \"Nothing executes until an action, so Spark can optimize the whole chain via Catalyst and prune columns/partitions before reading. The performance mistakes are triggering actions you don't need — count() to 'check', collect() to driver memory, or show() inside a loop.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "Compute all aggregates in one agg() — I've seen code do five separate groupBy+join passes for five metrics, which is five shuffles instead of one. Also: countDistinct is expensive (it can't be partially aggregated as cheaply as sum/count); for huge cardinality with tolerance for error, approx_count_distinct (HyperLogLog) is dramatically faster.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"When one side is small I broadcast it — that removes the shuffle of the big side entirely, which is usually where the time goes. Spark auto-broadcasts below the threshold, but the estimate is often wrong for freshly computed DataFrames, so I hint F.broadcast() explicitly.\" The failure mode to name: broadcasting a table that's too big OOMs the driver/executors — broadcast is for genuinely small dimensions, not 'medium' tables.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "Two things to flag: (1) a Window without partitionBy puts everything in one partition on one executor — fine for tiny data, catastrophic at scale; if you need a global order, reconsider the requirement. (2) Just like SQL, orderBy without a frame defaults to a range frame up to the current row — specify rowsBetween for a true row-count running total.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"Partition by something low-cardinality and commonly filtered — usually date. Partitioning by account_id or transaction_id is the small-files disaster: millions of directories with one tiny file each, and the metadata overhead alone kills reads. If files come out too small I coalesce/repartition before writing, or compact them.\" Naming the small-files problem unprompted is a strong DE signal.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"coalesce is cheap because it doesn't shuffle — I use it to cut output-file count before writing. But coalesce(1) on a big DataFrame is a trap: it forces all data through one task/executor and OOMs or crawls. If I need one file I'd rather write partitioned and compact, or repartition then write. repartition when I actually need even redistribution.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "Clarify the edge cases OUT LOUD before coding: how to handle lists (index them? join them?), empty dicts (drop or keep as leaf?), and key collisions. In production I wouldn't hand-roll this at scale — Spark's from_json + explode + selectExpr with a known schema is the right tool. The pure-Python version is for small payloads or the interview warmup itself.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "The point to make: 'if x not in a list' is O(n) so the whole loop is O(n^2) — a set makes membership O(1) and the whole thing O(n). If order doesn't matter, list(set(items)) is fine; if it does, the seen-set or dict.fromkeys preserves it. Stating the complexity and the order-preservation tradeoff is what they're checking.",
      followups: [
  ...
  cards: [
    {
      ...
      noteLabel: "Model answer:",
      note: "\"They express the same relational logic, so I choose by context. If the data's already in the warehouse (Snowflake) and the transform is set-based, I do it in SQL — often via dbt — because it's declarative, testable, versioned, and the warehouse optimizes it; no cluster to run. I reach for PySpark when data lives in the lake (S3/Parquet), when I need procedural logic or ML feature code that's awkward in SQL, when I'm processing before it ever lands in a warehouse, or when volumes/cost favor Spark's distributed compute over warehouse credits. At Amex I'd land and heavily transform in PySpark on the lake, then do the final set-based marts in Snowflake SQL. The anti-pattern is dogma either way.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"First I EXPLAIN it and find the dominant node — usually a full scan or a big shuffle. Then, cheapest first: (1) read less — add a partition-pruning predicate and project only needed columns instead of SELECT *; make sure I'm not wrapping the partition column in a function, which defeats pruning. (2) Filter before joining, not after. (3) Fix the join — broadcast a small dimension to kill a shuffle, or check for a fan-out from a non-unique key that's inflating the row count. (4) Check for skew — one straggler doing all the work. (5) Only then consider indexes (OLTP) or clustering/sort keys (warehouse). I verify each change against the plan and the runtime, not by faith.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"I open the Spark UI and look at the stages. (1) Skew — one or two tasks take 10x the others; that's a hot key, fix with AQE / broadcast / salting. (2) Small files — thousands of tiny input files means huge task overhead; compact them or repartition. (3) Excessive shuffles — every wide transform (join, groupBy, distinct) shuffles; I check whether I can broadcast, reduce the number of shuffles, or avoid re-shuffling already-partitioned data. (4) Driver bottleneck — a collect() pulling everything to the driver, or a Python UDF serializing row-by-row; rewrite with native functions. (5) Spill — tasks spilling to disk means partitions too big / not enough memory. I confirm with the UI's task-time distribution and shuffle/spill metrics rather than guessing.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"For pure Python I'll give the Big-O and the data-structure choice driving it — e.g. a set lookup makes dedup O(n) instead of O(n^2). For SQL/Spark I reason about the physical plan: a hash aggregate is about O(n), a sort is O(n log n), a self-join is O(n^2) within a partition, and the dominant real cost is usually the shuffle — how much data crosses the network — not CPU. So 'complexity' for a distributed job means 'how much data moves and how evenly', which is why skew and broadcast matter more than the theoretical bound.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"SQL is the wrong tool for: iterative/procedural logic and complex control flow (though recursive CTEs cover hierarchies); heavy string/JSON parsing of messy semi-structured data — Spark or Python is cleaner; anything needing external calls, ML training, or stateful streaming logic; and truly dynamic schemas (unknown columns) where you'd be generating SQL. I also avoid pushing enormous row-by-row transformations through SQL when the logic is more naturally expressed and tested in code. The heuristic: if it's set-based and the data's in a warehouse, SQL wins; if it's procedural, iterative, or the data isn't relational yet, drop to Python/Spark.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"ROW_NUMBER over PARTITION BY the business key, ORDER BY the version/updated timestamp descending, keep rn = 1 — in Snowflake I collapse it with QUALIFY. The part people miss: if two rows share the key AND the timestamp, I add a deterministic tiebreaker (source file sequence, ingestion order) so the result is stable across reruns — otherwise the pipeline is non-deterministic and dbt/incremental logic can flip-flop. I'd also confirm whether soft-deleted records should be excluded, and whether 'latest' means latest event time or latest ingestion time — they differ with late-arriving data.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"GROUP BY collapses each group into one summary row and you lose the detail. A window function keeps every original row and attaches a computed value calculated over a related set of rows — the 'window' defined by PARTITION BY (the group) and ORDER BY (the sequence). So 'total spend per account' is GROUP BY — one row per account. But 'each transaction, plus that account's running total' needs a window — I keep all transactions and add the cumulative sum. Anytime the requirement is 'per row, relative to its neighbors' — rank, previous value, running total — it's a window; anytime it's 'one summary per group', it's GROUP BY.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"I start from the actual latency SLA and the cost of being late. If the business genuinely needs sub-minute reaction — fraud scoring on a live transaction, real-time risk limits — that's streaming (Kafka + Spark Structured Streaming / Flink). If 'real-time' really means 'within an hour' — most reporting and reconciliation — a frequent micro-batch is far simpler to build, test, backfill, and operate, and it's cheaper. Streaming adds state management, watermarking for late data, exactly-once concerns, and 24/7 ops. My default is the largest batch interval that still meets the SLA, and I only pay the streaming complexity tax where the latency requirement truly demands it. At Amex, fraud is streaming; most customer/risk marts are batch.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"Layered checks. Before shipping: reconcile row counts and control totals against the source (does SUM(amount) tie out?), assert the grain — the primary key is unique and not null (GROUP BY key HAVING COUNT(*) > 1 must return nothing), check referential integrity (no orphaned foreign keys), and spot-check a few known records end to end. Then I make these tests automated and continuous — dbt tests (unique, not_null, relationships, accepted_values) or Great Expectations in the pipeline — so a bad upstream change fails the run instead of silently corrupting a mart. For the transform logic itself I'll unit-test the tricky bits (the dedup tiebreaker, the sessionization boundary) on small fixtures. The mindset: assume the data will break and make the pipeline tell me, rather than a stakeholder finding it.\"",
      followups: [
