  ...
  cards: [
    {
      ...
      note: "Airflow is the control plane above your compute: it triggers Spark/EMR/Databricks jobs, dbt runs, warehouse loads, and Kafka-adjacent batch jobs, wiring them into dependency chains with retries, SLAs, and alerting. At American Express this looked like orchestrating batch + near-real-time transaction and risk pipelines with hard SLAs and data contracts; the heavy lifting lived in Spark and the warehouse, Airflow owned sequencing, dependencies, and failure handling."
    },
    {
      ...
      note: "The migration story interviewers love: replacing a pile of cron + bash + 'did it run?' Slack messages with Airflow DAGs. At Cedar Gate I migrated legacy batch scripts to Airflow + dbt and improved throughput ~35% — the win wasn't speed of any one job, it was parallelism, dependency-aware scheduling, retries instead of silent failures, and visibility that eliminated manual babysitting."
    },
    {
    ...
    },
    {
      ...
      note: "Interview tell: a candidate who puts a Variable.get() or a requests.get() at module scope and is surprised the DAG is slow / the external API is getting hammered. Seniors know the file is re-parsed constantly and keep the module scope inert."
    }
  ]
},
  ...
  cards: [
    {
      ...
      note: "Always set start_date as a static, timezone-aware datetime — never datetime.now(). A dynamic start_date makes the set of intervals shift on every parse, which breaks scheduling in bizarre ways. This is a classic gotcha interviewers plant.",
      followups: [
    ...
    },
    {
      ...
      note: "TaskFlow and classic operators mix freely in one DAG — you can pass a @task output to a classic operator and vice versa. Use TaskFlow for Python glue; use provider operators (SparkSubmit, dbt, BigQuery, etc.) for the heavy integrations.",
      followups: [
    ...
    },
    {
      ...
      note: "Contrast with the old anti-pattern: a for-loop at module scope calling an API to decide how many operators to create. That runs on every parse and makes the DAG structure unstable. Dynamic mapping keeps the parse cheap and the fan-out data-driven.",
      followups: [
    ...
    },
    {
      ...
      note: "This is the #1 thing interviewers probe on backfills: 'you backfill 90 days — does anything double up?' If your writes are DELETE+INSERT or MERGE scoped to {{ ds }}, the answer is confidently no. If they're plain INSERTs, you have a data-corruption incident.",
      followups: [
    ...
    },
    {
      ...
      note: "Jinja templating is evaluated at task RUN time using the run's context, so {{ ds }} is the logical date of THAT run — this is exactly what makes a backfilled run process the right historical window.",
      followups: [
    ...
    },
    {
      ...
      note: "ExternalTaskSensor's most common failure: schedules that don't line up on the same logical date, so the sensor waits for a run that will never exist. Datasets sidestep the entire timing-alignment problem.",
      followups: [
    ...
    },
    {
      ...
      note: "Interviewers probe this bullet hard: expect 'what broke when you migrated?' The honest answers — scripts that assumed 'now', appends that double-counted on the first backfill, and timing dependencies that were actually races — are exactly the idempotency/determinism lessons from the previous cards.",
      followups: [
...
scheduling: {
  ...
  cards: [
    ...
    },
    {
      ...
      note: "Interview signal: a candidate who says 'execution_date is when the DAG runs' has not operated Airflow. The correct statement is 'logical_date is the start of the data interval; the run fires at the end of that interval.' This single misunderstanding causes the most off-by-one-day data bugs in Airflow.",
      followups: [
    ...
    },
    {
      ...
      note: "'How do you backfill safely?' expects: idempotent + deterministic tasks, a bounded date range, throttled concurrency (max_active_runs), verification that source data for those dates still exists, and awareness of downstream load. 'I set catchup=True' is the wrong answer.",
      followups: [
    ...
    },
    {
      ...
      note: "Sensor starvation is a favorite scenario: 'your DAGs all hang and nothing runs, but there are no errors.' Diagnosis: dozens of poke-mode sensors holding every worker slot. Fix: reschedule mode, deferrable operators, and timeouts.",
      followups: [
    ...
    },
    {
      ...
      note: "Requires the triggerer process to be running. If you enable deferrable operators but never start a triggerer, tasks defer and hang forever — a subtle deployment gotcha.",
      followups: [
    ...
    },
    {
      ...
      note: "SLA gotcha to name: SLAs are not evaluated for manually triggered / backfill runs, only scheduled ones — so 'I triggered it and no SLA alert fired' is expected behavior, not a bug.",
      followups: [
...
internals: {
  ...
  cards: [
    ...
    },
    {
      ...
      note: "Trade-off to name crisply: Celery = warm workers, low latency, pay for idle. Kubernetes = per-task pods, elastic to zero, higher per-task startup latency. There's no universally right choice — match it to workload shape.",
      followups: [
    ...
    },
    {
      ...
      note: "The interview trap: 'how do you pass a big dataset between tasks via XCom?' The senior answer is 'you don't — XCom lives in the metadata DB and is for small values; you stage the data in object storage and pass the path, or use a custom XCom backend backed by S3.'",
      followups: [
    ...
    },
    {
      ...
      note: "Top-level Variable.get() is a classic performance footgun: it fires a metadata-DB (or secrets-backend) call on every DAG parse for every file that does it. Keep it inside tasks or use templated var references.",
      followups: [
    ...
    },
    {
      ...
      note: "Pools are the answer to 'a backfill of DAG X keeps knocking over the shared warehouse.' Cap concurrent access with a pool sized to what the downstream can take, and prioritize live SLA work over the backfill.",
      followups: [
    ...
    },
    {
      ...
      note: "Retries are only safe because your tasks are idempotent (see the DAGs tab). Retrying a non-idempotent task just multiplies the corruption. The two concepts are inseparable at senior level.",
      followups: [
  ...
  cards: [
    {
      ...
      note: "\"The scheduler continually parses DAG files and evaluates each DAG's schedule. When a data interval completes, it creates a DAG run for that interval and the associated task instances in the metadata DB — note the run fires at the END of its interval, so a daily run for the 9th executes just after midnight on the 10th. The scheduler then checks each task instance's dependencies and trigger rules; when one is ready, it queues it to the executor. The executor (Local/Celery/Kubernetes) runs the task on a worker, which executes the code and writes state and any small XComs back to the metadata DB. Retries fire on failure per the retry policy; deferrable operators hand their wait to the triggerer to free the slot. The webserver just reflects the metadata DB. Everything coordinates through that database, which is why it's the critical component.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"Idempotent means running the task twice for the same data interval leaves the same end state as running it once. It matters because Airflow WILL rerun tasks — retries, backfills, catchup, manual clears — so any non-idempotent write silently corrupts data. Concretely: instead of INSERT INTO target SELECT …, I scope the write to the run's interval and make it replace-not-append — DELETE WHERE dt = '{{ ds }}' then INSERT, or MERGE/UPSERT on a business key, or overwrite the dt= partition in object storage. I also make the task deterministic by templating off {{ ds }} / data_interval_start instead of datetime.now(), so a backfilled run reads the correct historical window. Together — idempotent plus deterministic — a rerun is always both safe and correct.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"First, preconditions: the tasks must be idempotent and deterministic — templated off {{ ds }}, writing via DELETE+INSERT or MERGE per partition — otherwise a backfill duplicates or corrupts data. Then I run a bounded backfill with `airflow dags backfill --start-date … --end-date …` (or clear the specific runs), NOT by flipping catchup=True, which would stampede everything at once. I throttle it: lower max_active_runs (say 3) and route heavy tasks through a pool sized to what the warehouse can take, so I don't knock over shared downstreams, and I set priority so live SLA work still wins slots. I verify source data for those dates still exists, then spot-check row counts / a checksum on a few reprocessed partitions. If day N depends on N-1 I'd use depends_on_past and go oldest-first; otherwise I let them parallelize within the throttle.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"Airflow can't promise exactly-once execution — retries, clears, and overlapping runs mean a task may attempt more than once — so the guarantee has to live in the task's effect, not in Airflow. If the work is a data write, I make it idempotent (MERGE / overwrite the {{ ds }} partition) so a second run is a no-op. If it's a genuinely non-idempotent side effect — say, submitting a payment or sending an external event — I add an idempotency key derived from the dag_id + task_id + logical_date and dedup against it in the target system (or take a lock keyed on that), so a retry is recognized and ignored. I'd also use max_active_runs=1 (and depends_on_past if needed) to prevent two runs of the same interval overlapping. The principle is the same as idempotent Kafka consumers: make reprocessing harmless rather than pretending it can't happen.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"The name misleads everyone: execution_date is NOT when the DAG runs — it's the START of the data interval the run processes. A scheduled run fires at the END of its interval, because you can't process a period until it's complete. So a @daily DAG with logical_date 2026-08-09 actually executes just after midnight on the 10th, and {{ ds }} = 2026-08-09 is the day of DATA, not the day of execution. That's why you template off {{ ds }} directly with no date math — it already IS the interval to process. Modern Airflow renamed execution_date to logical_date and exposed data_interval_start / data_interval_end explicitly precisely to kill this confusion. The classic bug it causes is people adding a day to 'fix' an off-by-one that was never wrong.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"I'd localize it. First, is the run even being created on time? If DAG parsing is slow — expensive top-level code, thousands of DAGs, slow imports — the scheduler lags creating runs, so tasks start minutes late; I'd check scheduler metrics and DAG parse times and move any top-level work into tasks. Second, is the task created but stuck 'queued'? Then it's a concurrency ceiling: global parallelism maxed, the DAG at max_active_tasks, max_active_runs capping intervals, or a full pool — I check each in that order. Third, are workers saturated (Celery queue depth / no free K8s capacity)? Fourth, is it a sensor eating slots (poke-mode starvation)? I'd confirm with the scheduler logs, the task instance's state and 'why not scheduled' details, and the executor/queue metrics. 'Starts late' is almost always parse pressure or a concurrency limit, not the schedule itself.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"Classic symptom of worker-slot exhaustion, usually sensor starvation. Poke-mode sensors hold a worker slot the entire time they wait; if you have many long-running sensors, they occupy every slot, so no real tasks can be scheduled — the system deadlocks itself with zero errors because nothing failed, everything's just waiting. The fix: switch long-waiting sensors to mode='reschedule' so they release the slot between checks, or better, use deferrable operators that offload the wait to the triggerer and free the slot entirely — that scales to thousands of concurrent waits. I'd also set timeouts so no sensor can wait forever, and check global parallelism / pool sizing. Longer term, replace 'wait for the file' sensors with Dataset-driven scheduling so the consumer runs on the produce event instead of polling.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"Dynamic task mapping — .expand(). An upstream task returns a list at runtime (e.g. the files that landed today, or partitions from a query), and the mapped task runs once per element as indexed, individually-retryable instances. The key reason it's right: the count is decided by DATA at run time, with no code running at parse time. The old anti-pattern was a for-loop at module scope that hit an API to decide how many operators to create — that runs on every scheduler parse, hammers the external system, and makes the DAG structure unstable and slow to parse. With .expand() the parse stays cheap and inert while the fan-out is data-driven, visible in the UI, and each mapped instance can be retried on its own. I can also cap the mapped concurrency so a 500-way fan-out doesn't overwhelm downstream.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"Testing has layers. Cheapest: a DAG-validation/import test that parses every DAG and asserts no import errors and no cycles — catches most breakages in CI. Then unit tests: with the TaskFlow API my task logic is a plain Python function, so I test it directly with sample inputs, no Airflow needed — which is a big reason to keep business logic in testable functions rather than sprawling operators. Then integration tests with `airflow tasks test dag_id task_id date` to run a single task instance against real (or sandboxed) systems for a given logical date. On the second question: no — I wouldn't do heavy transformation inside Airflow tasks. Airflow is the orchestrator; the transform belongs in the warehouse (SQL/dbt) or Spark, which are built for set-based/distributed compute, give better lineage and testing, and keep Airflow tasks as thin, idempotent triggers. Pulling data into a Python task to transform it doesn't scale and abuses the metadata DB via XCom.\"",
      followups: [
