  ...
  cards: [
    {
      ...
      note: "As a DE, you meet Mongo as a source far more than as a warehouse.<br><br>It's the operational store behind a service. The data is semi-structured and schema-flexible: FHIR resources, event payloads, risk profiles. You have to land it into Snowflake or S3 via CDC (change streams or Debezium).<br><br>The recurring job is un-nesting documents into columnar tables.<br><br>Mongo is a great operational and serving store, but a poor analytical one. Know both sides."
    },
    {
      ...
      note: "Modern caveat: the line has blurred.<br><br>Postgres now has JSONB, which gives you document-ish columns. Mongo added multi-document ACID transactions in 4.0 and later.<br><br>So the decision is rarely 'impossible in the other.' It's which one makes the dominant access pattern cheap and the rare one merely possible."
    },
    {
      ...
      note: "Interview-ready one-liner: 'MongoDB gives per-operation tunable consistency and durability inside a CP replica set. It's strong by default from the primary and eventual from secondaries. Write concern controls how many nodes must acknowledge before a write counts as safe.'"
    },
    {
    ...
    },
    {
      ...
      note: "Attach validation with db.createCollection(name, { validator: { $jsonSchema: {...} } }).<br><br>Use validationAction:'warn' first so you can observe violations before you enforce. Flip straight to 'error' on a live collection and you reject writes you didn't know were happening."
    }
  ]
},
  ...
  cards: [
    {
      ...
      note: "Rule of thumb interviewers like: 'What data is displayed or updated together, and how big or unbounded can it get?'<br><br>Those two questions decide almost every embed-vs-reference call.",
      followups: [
    ...
    },
    {
      ...
      note: "The default bias in MongoDB is embed. It's what makes Mongo fast.<br><br>Reach for referencing only when one of the concrete triggers above fires.<br><br>'When in doubt, embed, but watch the array bounds' is a defensible senior stance.",
      followups: [
    ...
    },
    {
      ...
      note: "Watch for this on the read side too.<br><br>A projection that excludes a giant array ({ readings: 0 }) is a band-aid. The document is still huge on disk and in cache.<br><br>The real fix is not storing it embedded.",
      followups: [
    ...
    },
    {
      ...
      note: "Cap the bucket by time or by count, whichever comes first. For example, flush at 200 entries.<br><br>That way a burst can't blow the 16MB limit inside a single window.",
      followups: [
    ...
    },
    {
      ...
      note: "In healthcare and finance, snapshotting values as of the event is often the correct behavior, not a compromise. Think name, address, or plan at time of service.<br><br>The record must reflect what was true then. Extended reference and point-in-time correctness align here.",
      followups: [
    ...
    },
    {
      ...
      note: "The aggregation-pipeline form of update ([ {$set: ...} ]) lets a migration reference other fields of the same document. Plain update operators can't.<br><br>It's handy for exactly these in-place transforms.",
      followups: [
...
queries: {
  ...
  cards: [
    ...
    },
    {
      ...
      note: "$addToSet vs $push: $addToSet dedups (set semantics), while $push always appends.<br><br>Reaching for $push and then complaining about duplicates is a common bug.",
      followups: [
    ...
    },
    {
      ...
      note: "Aggregations have a 100MB per-stage memory limit.<br><br>A big $group or $sort that exceeds it errors, unless you set allowDiskUse:true, which spills to disk and runs slower.<br><br>Hitting that limit is a signal: your $match isn't selective enough, or you need a supporting index.",
      followups: [
    ...
    },
    {
      ...
      note: "Reduce $lookup cost by $match-ing hard before the $lookup, so you join few documents instead of the whole collection.<br><br>A $lookup at the top of a pipeline over an unfiltered collection is a classic slow-query cause.",
      followups: [
    ...
    },
    {
      ...
      note: "Interview trap: 'MongoDB isn't ACID.'<br><br>The correct answer: single-document ops always were atomic. Multi-document ACID transactions exist since 4.0, but they're meant for the exceptions. A well-modeled schema needs them rarely.",
      followups: [
    ...
    },
    {
      ...
      note: "Change streams need the resumeToken persisted downstream.<br><br>On restart you resumeAfter(token) so you don't miss or double-read events. It's exactly the offset-management discipline of any streaming pipeline.",
      followups: [
...
indexing: {
  ...
  cards: [
    ...
    },
    {
      ...
      note: "See a SORT stage in explain() despite an index? Your sort field is likely misplaced relative to ESR.<br><br>The index narrowed the match but couldn't supply order. Reorder to E-S-R and the SORT disappears.",
      followups: [
    ...
    },
    {
      ...
      note: "explain('allPlansExecution') shows the rejected plans too. That's useful when the optimizer picked a surprising index.<br><br>MongoDB caches the winning plan per query shape, so a plan can go stale after the data distribution changes.",
      followups: [
    ...
    },
    {
      ...
      note: "TTL granularity is about 60s, because a background sweeper does the work. So deletion isn't instant at the expiry second.<br><br>That's fine for retention, but not for precise scheduling.",
      followups: [
    ...
    },
    {
      ...
      note: "Rule of thumb: if adding an index doesn't demonstrably help a real query, don't add it. Check with explain.<br><br>And prefer one compound index over three single-field indexes when a prefix relationship exists.",
      followups: [
  ...
  cards: [
    {
      ...
      note: "The oplog is the CDC backbone: change streams and Debezium tail it.<br><br>Size the oplog for your peak write burst plus your longest expected secondary downtime. Otherwise you risk resyncs and gaps in downstream CDC.",
      followups: [
    ...
    },
    {
      ...
      note: "A write acknowledged at w:1 can still be lost. If it hasn't replicated when the primary crashes, it gets rolled back when that node rejoins.<br><br>That's the classic 'my write vanished' incident. w:'majority' prevents it.",
      followups: [
    ...
    },
    {
      ...
      note: "The 'read-your-write' bug: the app writes to the primary, then immediately reads from a secondary and gets nothing, because replication hasn't caught up.<br><br>Either read from the primary for that flow, or use causal consistency, which is a session guarantee.",
      followups: [
    ...
    },
    {
      ...
      note: "Don't shard prematurely.<br><br>Sharding adds real operational complexity: config servers, mongos, a balancer, and harder backup and restore.<br><br>A well-indexed replica set on decent hardware handles a lot. Shard when you've genuinely outgrown vertical scale, not by default.",
      followups: [
    ...
    },
    {
      ...
      note: "The three tests to say out loud: cardinality (can it split into many chunks?), frequency (does one value dominate into an unsplittable hotspot?), and monotonicity (do new writes all target one chunk?).<br><br>Name all three, give the hashed-vs-ranged tradeoff, and you've nailed the hardest MongoDB scaling question.",
      followups: [
  ...
  cards: [
    {
      ...
      note: "\"I wouldn't use MongoDB when the workload is dominated by complex multi-entity transactions and strong invariants. Think moving money between accounts, or anything needing referential integrity across many tables. A relational DB gives me ACID and constraints natively.<br><br>I'd also avoid it for ad-hoc analytical SQL with lots of joins across large independent entities. Mongo has no join optimizer, so that belongs in a warehouse like Snowflake.<br><br>And I'd avoid it when the data is genuinely tabular and the schema is stable. I'd just be paying for flexibility I don't use.<br><br>Mongo shines when the entity is a nested tree read as a unit, when the schema is evolving, or when I need horizontal write scale.<br><br>My caveat: the line has blurred. Postgres has JSONB now, and Mongo has multi-document transactions. So it's about which tool makes the dominant access pattern cheap, not what's strictly impossible in the other.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"First I'd ask how it's read and how it grows.<br><br>Demographics, current meds, and allergies I'd embed in the patient document. They're bounded, owned by the patient, and shown together on the summary screen. So it's one read, and updates are single-document atomic.<br><br>Claims I'd reference in their own collection with patientId indexed. A patient accumulates claims without bound, so embedding them would grow the document toward the 16MB limit and drag the whole array on every read.<br><br>If the common screen shows 'last 5 claims,' I'd use the subset pattern: keep the recent 5 embedded and the full history referenced. That keeps the dominant read a single lookup.<br><br>The trap I'd avoid is embedding an unbounded array because it 'reads faster today.' It's a time bomb against the document-size limit.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"A replica set does not scale writes. Every write goes to the single primary. Secondaries only scale reads and provide HA.<br><br>To scale writes, you shard: partition the collection across multiple shards by a shard key. Each shard is its own replica set, and a mongos router directs writes to the right shard. That distributes write load horizontally.<br><br>But it only works if the shard key spreads writes evenly. A monotonic key like a timestamp or ObjectId sends every new write to the single highest chunk. One shard takes all the load while the others idle, and you've added complexity for no scaling.<br><br>So 'how does Mongo scale writes' is really 'pick a shard key with high cardinality, even frequency, and non-monotonic write distribution.' That's often a hashed key, or a compound key leading with a well-distributed field.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"I'd reject the tempting keys first.<br><br>Timestamp or a monotonic transactionId is high cardinality but monotonic, so every new write hits one shard. That's a write hotspot, exactly the wrong choice for a high-volume stream.<br><br>A low-cardinality field like currency or merchant category can't split into enough chunks, so it can't spread.<br><br>What I'd actually pick depends on the dominant read. If we mostly query 'all transactions for an account,' I'd use accountId. It's high cardinality, evenly distributed if no single account dominates, and non-monotonic, so reads by account are targeted to one shard.<br><br>If a few whale accounts dominate volume, that account's chunk hotspots and can't be split, since it's a single key value. So I'd go compound, { accountId: 1, timestamp: 1 }, to spread a hot account across chunks while keeping some time locality. Or I'd hash accountId if I don't need range queries.<br><br>I'd state the tradeoff: hashing kills range-scan locality, and the shard key is effectively permanent. So I'd validate it against real query and volume distributions before committing.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"Yes, with nuance.<br><br>Single-document operations have always been atomic in MongoDB. Because a document can nest related data, a lot of what needs a multi-row transaction in a relational DB is a single-document atomic update here. That's a big part of why the model works.<br><br>Since 4.0 on replica sets and 4.2 on sharded clusters, MongoDB also supports multi-document ACID transactions with snapshot isolation, for the cases that genuinely span documents.<br><br>But they're the exception, not the pattern. They hold locks, have a ~60-second default limit, can abort on write conflicts so you must retry, and cost more across shards.<br><br>The senior point: if I find myself reaching for transactions constantly, my schema is probably too normalized. Good document modeling makes most changes single-document and atomic for free.<br><br>So: atomic per-document always, full multi-document ACID available since 4.0, used sparingly by design.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"I start from access patterns, not entities.<br><br>Take claims. The dominant reads are 'show a claim with its line items and the provider name' and 'list a patient's recent claims.'<br><br>Line items I embed in the claim: bounded, always read together, single-document atomic updates.<br><br>The provider I reference by id, but I use an extended reference. I snapshot the provider name and NPI on the claim so the list view needs no $lookup. In healthcare that snapshot is actually correct, because the record should show the provider as of service date.<br><br>A patient's claims I reference in a claims collection with patientId indexed, since the history is unbounded. I use the subset pattern to keep the recent few on the patient summary.<br><br>For a batch total I'd use the computed pattern: maintain totalAmount with $inc rather than aggregating on every read, with a nightly reconciliation to correct drift.<br><br>And I'd stamp a schemaVersion so I can evolve the shape without a big-bang migration.<br><br>The through-line: embed by default, reference when unbounded or shared, and duplicate small display fields deliberately to make the dominant read a single lookup.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"I'd run it through explain('executionStats') and read it top-down.<br><br>First, is the leading stage an IXSCAN or a COLLSCAN? A COLLSCAN means my $match isn't hitting an index. I'd check that $match is the first stage, because a $match after $group or $unwind can't use an index and runs on the whole in-memory stream. I'd also check that a supporting index exists in ESR order.<br><br>Second, is there a SORT stage? That's a blocking in-memory sort. I'd add or reorder a compound index so the index supplies the order.<br><br>Third, is there a $lookup near the top over an unfiltered collection, or on an unindexed foreign field? That's a per-document query. I'd $match hard before the $lookup and index the foreign field, or question whether that join belongs in Mongo at all versus a warehouse.<br><br>Fourth, if it's erroring at the 100MB stage limit, or I've set allowDiskUse, that tells me the filter isn't selective enough.<br><br>I'd confirm the fix by comparing totalDocsExamined to nReturned. I want them close.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"MongoDB is an operational and serving store, not an analytical one. There's no columnar storage and no join optimizer, aggregations run on the live cluster competing with production traffic, and cross-collection joins over large data are expensive.<br><br>So for anything beyond light operational rollups, I don't run analytics in Mongo.<br><br>The pattern I'd build: capture changes with change streams, or Debezium's Mongo connector, which tails the oplog. Stream them into Kafka, and land them in Snowflake or S3, un-nesting the documents into flat columnar tables. That's exactly what $unwind and $project do, but I'd rather do the heavy lifting downstream.<br><br>Then analysts join and aggregate in the warehouse, where the query planner is built for it, and production Mongo is never touched by a heavy report.<br><br>If I must query Mongo directly for ops dashboards, I'd point those reads at a secondary with secondaryPreferred so they don't load the write primary. I'd accept a little replication-lag staleness.<br><br>The line: operational reads and single-entity lookups in Mongo, analytical joins and aggregations in the warehouse, CDC as the bridge.\"",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"For a one-time or slowly-changing load, a batched extract with bulkWrite-style paging off a secondary works.<br><br>But for freshness at scale, you don't full-scan repeatedly. You use CDC. MongoDB's change streams expose a resumable feed of inserts, updates, and deletes built on the replica-set oplog. Each event carries a resumeToken I persist downstream, so on restart I resumeAfter it and neither miss nor double-read. It's the same offset discipline as Kafka.<br><br>In practice I'd run Debezium's Mongo connector, which is a managed change-stream consumer, land events in Kafka, then sink to Snowflake or S3.<br><br>The hard part is that Mongo is schemaless, so the pipeline owns the schema. Fields are sometimes absent or type-varying, and arrays need un-nesting into rows.<br><br>I'd lean on the schemaVersion field if the source uses it, enforce a target schema in the warehouse, and handle deletes explicitly, since a delete event carries only the _id.<br><br>I'd also size the oplog generously. If a secondary or the CDC consumer falls behind the oplog window, I get a gap and a costly resync.\"",
      followups: [
