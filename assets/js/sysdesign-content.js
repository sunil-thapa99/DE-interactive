  ...
  cards: [
    {
      ...
      note: "Requirements first, architecture second. 'What's the freshness SLA and is a duplicate acceptable?' is the highest-signal opening question in the entire round."
    },
    {
      ...
      note: "Sources → Ingestion (CDC / Kafka / batch) → Landing/Bronze (raw, immutable) → Silver (cleaned, conformed, deduped) → Gold (business marts / aggregates) → Serve (BI, ML, reverse-ETL). Almost every DE design is a variation on this line — memorize it and specialize per prompt."
    },
    {
      ...
      noteLabel: "The trap:",
      note: "Reaching for Kafka/Flink on a prompt whose SLA is 'next morning.' Streaming buys latency at the cost of operational complexity, harder reprocessing, and 24/7 on-call. Only pay for it when the SLA demands it — and say so explicitly."
    },
    {
      ...
      noteLabel: "Remember:",
      note: "There is rarely one right architecture — there's the right architecture FOR THE STATED REQUIREMENTS. Change the SLA or the budget and the answer changes. Make that dependency explicit and you sound like someone who has actually shipped these systems."
    }
  ]
},
  ...
  cards: [
    {
      ...
      note: "Transaction events → Kafka (partition by card/account so per-card events stay ordered) → Flink/Spark Structured Streaming computes velocity features over sliding windows + calls/serves the model → score published back to a Kafka topic for the decision service AND upserted to a feature/store. In parallel: Kafka → S3 raw (bronze, immutable, PCI-encrypted) → PySpark/dbt clean+dedupe by transaction_id (silver) → Snowflake gold marts for analysts + labeled training sets. Airflow orchestrates the batch/training side. Arrow sketch: card swipe → Kafka (keyed by card) → Flink windowed features + model score → decision topic + feature store; Kafka → S3 bronze → PySpark silver (dedupe by txn_id) → Snowflake gold → BI + model retraining. Exactly-once concern handled by at-least-once + idempotent upsert keyed by txn_id, so replay/retry never double-counts.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Reference design:",
      note: "Multi-protocol ingestion: SFTP/MLLP/FHIR/EDI landers write RAW messages to S3 bronze (immutable, encrypted, per-source partitioned) → a parsing layer (HL7/FHIR/EDI parsers, often on Spark or a dedicated parser service) normalizes each format into a common canonical model in silver, deduping by message/claim ID → identity resolution (EMPI) links members/providers → dbt builds the dimensional gold layer (fact_claim at claim-line grain, dim_member with SCD Type 2, dim_provider, dim_date) in Snowflake/PostgreSQL → serve quality/risk/actuarial marts with row-level PHI access control. Arrow sketch: hospital SFTP/HL7/EDI → landing (raw to S3 bronze) → Spark/parser normalize to canonical (silver, dedupe by claim_id) → EMPI identity resolution → dbt star schema (fact_claim + SCD2 dims) in Snowflake → governed BI. Kafka/CDC used where sources push real-time HL7. HIPAA: encryption at rest/in transit, column-level PHI masking, full audit, de-identified analytics views.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Reference design:",
      note: "CDC from OLTP (Debezium) → Kafka → S3 bronze raw → dbt builds silver (cleaned, typed, deduped by PK) → gold star schema in Snowflake: fact_order_line at order-line grain (measures: quantity, amount) with FKs to dim_customer, dim_product, dim_date; dim_customer and dim_product use SCD Type 2 (surrogate key, effective_from/to, is_current) via dbt snapshots so a fact row joins to the dimension version valid at order time. Arrow sketch: OLTP → Debezium CDC → Kafka → S3 bronze → dbt silver (dedupe by PK) → dbt gold star schema (fact_order_line + SCD2 dims) in Snowflake → BI. Airflow triggers dbt daily. Quality gates: PK uniqueness on dims, fact-to-dim referential integrity, revenue reconciliation vs source.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Reference design:",
      note: "SDK/collector → Kafka (high partition count for parallelism; schema registry with BACKWARD compat for evolving events) → two consumers: (a) stream processor (Flink/Spark Structured Streaming) computes windowed metrics → served to a fast store (Druid/ClickHouse/Pinot or a real-time dashboard); (b) Kafka → S3 raw in Parquet, partitioned by dt/hour, with compaction to fix the small-files problem → dbt/Spark builds silver/gold aggregates in the lakehouse/warehouse for batch funnels + ML features. Arrow sketch: app SDK → Kafka (partitioned, schema registry) → Flink windowed metrics → ClickHouse/Druid live dashboards; Kafka → S3 Parquet (dt/hour partitions, compacted) → dbt/Spark gold → BI + ML. At-least-once + dedup by event_id where it matters; loss-tolerant elsewhere. Kappa-style: replay Kafka to backfill.",
      followups: [
    ...
    },
    {
      ...
      noteLabel: "Reference design:",
      note: "Debezium reads the Postgres WAL (log-based CDC — complete, ordered, near-zero source load) → publishes change events (with before/after images + op type) to Kafka, one topic per table, keyed by primary key (so per-key changes stay ordered), often compacted → a consumer (Kafka Connect JDBC/Snowflake sink, or Spark/dbt) applies changes as a MERGE into the warehouse silver table keyed by PK, handling deletes via tombstones → dbt builds gold on top. Arrow sketch: Postgres WAL → Debezium → Kafka (per-table, keyed by PK, compacted) → MERGE upsert into Snowflake silver (deletes via tombstone) → dbt gold → BI. Idempotent by construction: MERGE keyed by PK + newest-event-wins on ordering, so replays and retries converge. Initial load via Debezium incremental snapshot to avoid locking the source.",
      followups: [
  ...
  cards: [
    {
      ...
      note: "\"Aggregations must be driven by EVENT time (when it happened), not processing time (when we saw it), or a network delay silently corrupts the wrong window. I use watermarks: the engine tracks the max event time seen and assumes no data older than watermark minus a grace period will still arrive, so it holds a window open for that grace period before finalizing. The trade-off is direct: a longer grace period catches more late data but delays results and holds more state; a shorter one is faster and cheaper but drops stragglers. Data later than the grace period goes to a side-output/dead-letter for a batch correction rather than silently vanishing. In Spark Structured Streaming that's withWatermark; Flink has explicit watermark strategies and allowed lateness. The cost lever is grace period = state size, so I tune it to the real observed lateness distribution, not a guess.\""
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"Because I keep raw immutable in bronze partitioned by date, I can target exactly the affected window. I run the fixed transform on a SEPARATE compute pool at low priority so it doesn't starve the live pipeline, writing to the affected partitions with idempotent partition-overwrite — dynamic partition overwrite in Spark, REPLACE WHERE in Delta, or a dbt full-refresh scoped to the date range — so re-running replaces those partitions cleanly with no duplicates. For the serving layer I compute into a shadow location and do an atomic swap/partition-pointer switch, so live dashboards never see a half-written state. Because transforms are deterministic and sinks idempotent, unaffected rows come out byte-identical. If it's a Kappa/streaming setup, reprocessing is just replaying that slice of the Kafka log through the same code into the same idempotent sink. Zero downtime comes from never mutating the live path in place — you build the corrected data beside it and flip.\""
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"I don't rely on end-to-end exactly-once across systems — that's largely a myth once an external sink is involved. Instead I make duplicates HARMLESS: every transaction carries a stable business ID (txn_id), and the sink write is a MERGE/upsert keyed on it, so a retried batch, a consumer rebalance re-delivering a partition, or a full replay all converge to the same single row. On the produce side I enable the idempotent producer so retries don't create Kafka-level duplicates in the first place. For aggregations where I'm summing money, I dedup on txn_id BEFORE aggregating, or aggregate from the deduped silver table rather than the raw stream. The only time I reach for true Kafka transactions / two-phase commit is when the sink genuinely can't upsert and a duplicate is unfixable — like a non-idempotent external charge. Otherwise 'at-least-once + idempotent upsert keyed by txn_id' is simpler and just as correct, and I can prove it by replaying and showing the row count is unchanged.\""
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"I'd trace the spike through each stage and name the bottleneck and the lever. Ingestion: Kafka absorbs the burst as a buffer — that's the whole point of decoupling producers from consumers — but I need enough partitions up front because I can't cheaply add them to a keyed topic later, so I provision partition count for peak parallelism. Processing: the stream/batch cluster autoscales on lag or queue depth; if a hot key skews load I re-key or add sub-bucketing. Sink: often the real bottleneck — I batch writes, use bulk-load paths (Snowpipe/COPY over row-by-row), and let Kafka backpressure hold the excess rather than dropping it. The key insight is Kafka turns a spike into lag, not loss, so I scale consumers to burn down lag and the SLA degrades gracefully instead of the system falling over. Cost lever: autoscaling means I only pay for peak during the spike, and I set a max so a runaway doesn't blow the budget. I'd load-test at 10x beforehand so I know which component actually breaks first rather than guessing.\""
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"Idempotent means running it once or five times produces the same final state — which is what makes retries and backfills safe. Three properties get you there. First, immutable input: raw lands append-only in bronze partitioned by date, so a rerun reads the exact same bytes. Second, deterministic transforms: no dependence on wall-clock now() or random ordering that changes results between runs — same input must yield same output. Third, idempotent writes: partition-overwrite or MERGE/upsert keyed by a business key, never blind append, so re-writing a partition replaces rather than duplicates. With those three, a mid-DAG failure that Airflow retries, or a manual backfill of a date range, or a Kafka replay all land on identical correct data. The anti-pattern is an INSERT that appends on every run — the first retry doubles your data. I test it by running the same task twice and asserting the output is unchanged.\""
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"I decide from the freshness SLA and what decision the data drives, never from what's trendy. If the business acts in seconds — fraud scoring, live ops alerting — it's streaming: Kafka plus Flink or Spark Structured Streaming, and I accept the cost of managing state, watermarks, and 24/7 on-call. If the SLA is hourly or daily — most reporting and dimensional warehouses — it's batch: cheaper, simpler, trivially reprocessable, and I'm not paying for streaming infra and complexity that buy nothing. The middle ground is micro-batch — Spark Structured Streaming triggers, Snowpipe, dbt every few minutes — which hits 'near-real-time' at a fraction of true-streaming complexity and covers a lot of 'we want it fresh-ish' asks. The trap is reaching for Kafka/Flink on a next-morning SLA: that's latency you're not using, paid for in operational burden. So my answer is always 'what's the SLA and the decision?' and the architecture follows from that.\""
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"The root cause is an implicit, ungoverned schema. I put a schema registry in front with BACKWARD compatibility as the default, so additive changes (new optional field) are safe and consumers reading old data still work, while a breaking change — renaming or retyping a required field — is REJECTED at publish time instead of at 2am in prod. I formalize this as a data contract: the schema, semantics, and ownership, checked in CI so the producing team can't merge a breaking change. Compatibility direction dictates deploy order: BACKWARD means upgrade consumers first, FORWARD means producers first — knowing that is the difference between a smooth rollout and an outage. In the warehouse I use a table format that evolves cleanly (Iceberg/Delta add-column), and for truly breaking changes I version the topic/table (v2) and migrate consumers deliberately rather than mutating in place. The principle: schema is a contract enforced by tooling, not tribal knowledge.\""
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"Log-based CDC with Debezium reading the Postgres WAL — it captures every insert/update/delete in commit order with near-zero load on the source, unlike query-polling on updated_at which misses hard deletes and intermediate updates. Change events go to Kafka, one topic per table, KEYED BY PRIMARY KEY so all changes for a row land on the same partition and stay ordered. The sink applies them as a MERGE into the Snowflake silver table keyed on that PK, with newest-event-wins using the log sequence number / commit timestamp so an out-of-order redelivery can't overwrite newer state with older. Deletes are handled explicitly via tombstones so the warehouse reflects removals. It's idempotent by construction — MERGE keyed by PK means replays and retries converge to the same row. For the initial load I use Debezium's incremental snapshot so seeding a large table doesn't lock production. I'd monitor replication lag and alert on it, since a stalled connector is the usual failure. dbt builds gold on top of the synced silver.\""
    },
    {
      ...
      noteLabel: "Model answer:",
      note: "\"The classic 'our real-time and batch dashboards show different totals' incident comes from Lambda architecture: a speed layer and a batch layer computing the same metric in two separate codebases that inevitably drift. My default now is Kappa — ONE streaming pipeline is the source of truth, and 'reprocessing' means replaying the Kafka log through the SAME code into the same idempotent sink, so the historical and real-time numbers come from identical logic and can't disagree by construction. That requires enough Kafka retention (or tiered storage) to replay the needed history and idempotent sinks so a replay overwrites rather than duplicates. I'd only keep Lambda if the batch engine can compute something the stream engine genuinely can't at acceptable cost, and even then I'd share as much transformation logic as possible. If the SLA doesn't actually need real-time, I collapse to pure batch and the problem disappears entirely — two systems computing the same number is a cost and a correctness risk, so I only run two when the requirements truly force it.\""
    }
  ]
},
